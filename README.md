...
| 模型路线自动降级 | `LLM_PROVIDER=auto` 配合候选链可按 `openai:`、`deepseek:`、`bigmodel:`、`gemini:`、`mimo:` 等前缀路由；未配置 Key 或调用失败时按路线尝试后备模型。 |
...
机器人既可以回答问题，也可以在待办提醒、每日待办、订阅更新时主动发送消息。RSS、Todo 单次提醒和 Todo 每日提醒都先写入统一 Notification Outbox，再由后台 Worker 负责投递、重试和失败终态。
...
| Todo 编号与提醒 | 最近可见编号快照、按“第一条 / 刚才那条”继续操作、明确时间的单次提醒、X 分钟后提醒、每 N 分钟/小时重复提醒、每日个人待办提醒；当前不支持定时动态执行 Tool/命令 |
...
这意味着：换 API Key、旧兼容内置 Provider Base URL、默认模型或部署路径，优先改 `.env`；使用内置 Gemini 时配置 `GEMINI_API_KEY` 后即可在路线里写 `gemini:` 前缀；声明 MiMo 等公开 OpenAI-compatible provider 元数据、调整私聊 / 群聊用哪个 profile、是否允许 Tool Calling、允许哪些工具、最多跑几轮工具、输出预算多少，优先改 `agent.toml`。实际密钥始终以进程环境变量优先，dotenv 文件只补充缺失项。
