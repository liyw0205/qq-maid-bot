...
当前版本：**v0.11.0**。这一版已经完成统一 Agent Loop、可信工具结果编排、Todo 澄清与受限恢复、私聊最终回复流式发送，以及 QQ 原生 typing 支持。详细变更见 [CHANGELOG.md](./CHANGELOG.md)。
...
* 高风险 Todo 操作需要确认
* 澄清恢复只允许在原任务的候选边界内继续
* 群聊、slash 命令、文件处理和宿主机代码执行不会自动进入普通私聊 Agent Loop
