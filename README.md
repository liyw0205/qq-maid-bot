...
最少需要填写：`QQ_BOT_APP_ID`、`QQ_BOT_APP_SECRET`，以及默认模型路线实际引用到的 Provider API Key。默认 `config/agent.toml` 已显式声明私聊、群聊和辅助模型路线；`.env` 只保存 API Key、Base URL、旧兼容兜底模型和其他运行参数。使用第三方或自建兼容接口时，再配置对应的 Base URL。
