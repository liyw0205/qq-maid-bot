...
* `PromptConfig::load_chat_system_prompts(user_text)` 已在普通聊天链路接入上下文模块，
  顺序为：固定 prompt → WORLD_FILE → 上下文模块 → 旧成员映射。
...
pub enum CachePolicy {
    /// 长期稳定，适合作为缓存前缀。典型：maid_system、mode_rules、world、legacy_member_mapping。
    Stable,
...
| Stable | 旧成员映射配置 提示 | Stable | 文件不变则稳定 |
| Session | 旧成员身份提示（已移除） | Session | 会话内稳定 |
...
pub struct ContextEntry {
    pub id: String,
    pub source: &'static str,        // "prompt_dir" / "world" / "context_module" / "memory" / "session" / "request_time" / "legacy_member_mapping" / "member_identity"
...
* 现有 `PROMPT_DIR`、`WORLD_FILE`、旧成员映射配置 不改名、不迁移；
